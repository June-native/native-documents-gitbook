...
| --- | --- | --- | --- |
| `RateLimited` | Per-signer request rate exceeded (1000 req/s per signer); never admitted. HTTP `429`, carries `error.retry_after_ms`. | "Too many requests — retrying shortly." | Back off `retry_after_ms`, then resend the same signed action. The only safe resend. |
...
| `RateLimited` | The signer exceeded the per-signer request rate (1000/s over a 1-second window). HTTP `429`; includes `retry_after_ms`. |
