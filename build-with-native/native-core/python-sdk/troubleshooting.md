...
| `LocalValidationError: wallet 0x… is not an active agent for owner 0x…` (at construction) | The API wallet is not an approved agent on that account — it was revoked or replaced, or the `owner` is wrong. | Confirm with `info.agent_status(owner, agent)` / `info.user_agents(owner)`; mint a fresh API wallet in the web app if the key was rotated. |
| `LocalValidationError` at construction against the mainnet gateway | Mainnet is defined but blocked — an API wallet can only be approved on the testnet site. | Build from a `testnet` bundle (`Exchange.from_bundle(...)`), or point at `constants.TESTNET_API_URL`. |
| `DirectSignerIsActiveAgent` (rejected) | You built `Exchange(wallet, base_url)` with an API wallet key but no `owner`, so the SDK signed as a direct owner — but the gateway knows the key is an active agent, which may never sign as an owner. | Pass `owner=<accountAddress>`, or (simplest) use `Exchange.from_bundle(bundle)`, which sets the owner for you. |
| `AgentEpochMismatch` (rejected) | The signed `agent_epoch` was stale relative to the on-chain approval. | The SDK re-resolves the epoch from `userAgents` and retries **once** automatically. If it persists, the API wallet was revoked or re-approved — mint a new one in the app. |
| `WrongChainId` (rejected) | The key's network does not match the gateway you hit. | Use `from_bundle` — it picks the gateway from the bundle's `network`. Building by hand, match the endpoint to the bundle. |
...
* **Testnet only.** Constructing `Info` or `Exchange` against the mainnet gateway raises `LocalValidationError` at construction. Use a `testnet` bundle.
