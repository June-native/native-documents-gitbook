...
| `LocalValidationError: no order book available for … the market is unknown, or the gateway's query view is not ready yet` · `no ask/bid on … to derive a protection price from` | `protection_price` could not read a book. Either the market id does not exist, or no book has been published yet — which is every market for a few seconds after a restart, not just a typo. | Retry after a moment, or pass `ref_price=` to skip the book read. Catch it as `native_core.Error`; in 1.x this was a bare `KeyError` that `except Error` missed. |
...
| `SubscriptionError: Too many subscriptions` · `no answer to subscribe within …s` | A WebSocket subscribe was refused or went unanswered. Ten subscriptions per connection is the cap, and one connection per client IP. | Multiplex onto the one socket instead of opening more. Carries `.reason` and `.subscription`; see [Streaming](streaming.md). |
...
```python
from native_core import is_accepted, is_order_failed, leaf_error_code, order_oid
...
* **Check `found` before indexing a book.** `l2_book` omits `bids` and `asks` **entirely** when `found` is false, so `book["asks"]` raises a bare `KeyError` that `except Error` cannot catch. It happens for every market in the seconds after a restart, not just for a mistyped symbol.
