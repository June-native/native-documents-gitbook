...
* **On a `timeout`, branch on `error.code` before you decide.** `Unavailable` (HTTP 503) never reached a node — resubmit the same signed bytes, or you drop every write for as long as the condition lasts. Everything else may still land: reconcile by `cloid`, never resubmit under a new nonce. See [Handle outcomes & timeouts](handle-timeouts.md#reconciling-a-timeout).
