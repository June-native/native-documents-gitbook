...
You create one by generating an agent keypair locally and authorizing it with a single owner-signed `approveAgent` — sign it with your **main wallet** directly against the API, or let the Native web app do it and hand you a one-time **connection bundle** `{ network, accountAddress, agentPrivateKey }` with the agent key. Deposit from your main wallet to create the trading account first. See [API Access](api-access.md#access-model-api-wallets) for the full flow; revoke or rotate the agent any time.
